...
from dqt.json_manager import SubDictEnum, json_manager
...
class SettingsMenu:
    ...
    def display_option_list(self) -> None:
        ...
        settings: list[tuple[str, list[Any]]] = json_manager.get_subdict_ranges(self.chosen_menu)
        current_yindex: int = 0
        current_xindex: int = 0
        keys = list(map(lambda setting: setting[0], settings))
        selecting: bool = True
        
        while selecting:
            chosen_values: list = keys[current_yindex]
